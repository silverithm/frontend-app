...
| 17 | screens/chat_room_screen.dart | [~] | **정적그림자만 우선 해소**: `_buildMessageInput` 하단 입력바 Container(BoxShadow blurRadius10)를 widgets/chat/message_input.dart와 동일하게 상단 `AppSemanticColors.borderSubtle` 1px 보더로 교체(중복 그림자 제거). 말풍선 radius는 이미 AppBorderRadius 토큰 사용 중임을 확인(변경 불필요), 말풍선 색(`interactivePrimaryDefault`)도 brandDefault와 동일한 teal600으로 정합 확인. 날짜 구분선 UI는 이 화면에 존재하지 않음(해당 없음). shadcn버튼 5곳+AlertDialog 4곳은 이번 작업 범위에서 명시적으로 제외되어 미변경(다음 작업으로 이월). analyze 0 error, 로직 변경 없음. |
